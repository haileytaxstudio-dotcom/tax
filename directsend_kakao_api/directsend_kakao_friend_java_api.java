...
public class JavaKakaoApi{
	public static void main(String[] args) throws IOException {
		/* 발신프로필 / 예약 API 사용시 여기서부터 삭제후 이용 하시기 바랍니다. */
		 ...
		 * kakao_friend_type : 친구톡 종류 (텍스트/이미지)
		 ...
		 * template_ad : (광고)삽입 여부
		 * template_title : 템플릿 제목 (한/영 띄어쓰기 포함 15자)
		 * template_body : 템플릿 내용 (한/영 띄어쓰기 포함 1000자, 친구톡-이미지  한/영 띄어쓰기 포함 400자), 변수 문자열 사용 가능
		 *   변수 문자열 : #{이름} - 이름, #{휴대폰} - 휴대폰, #{비고1} - 비고1, #{비고2} - 비고2, #{비고3} - 비고3, #{비고4} - 비고4, #{비고5} - 비고5
		 * template_buttons : 템플릿 버튼
		 *   json array [{"linkType":"WL","name":"버튼명","linkM":"모바일링크","linkP":"PC 링크","linkI"=>"IOS 링크","linkA":"안드로이드 링크"},{"linkType":"AL","name":"버튼명","linkM":"모바일링크","linkP":"PC 링크","linkI":"IOS 링크","linkA":"안드로이드 링크"}]
		 *   linkType : 웹링크(WL) / 앱링크(AL) / 봇키워드(BK) / 메시지전달(MD)
		 *     linkType 웹링크(WL)인 경우 모바일 링크, PC 링크 필수 입력(http:// 포함 작성)
		 *     linkType 앱링크(AL)인 경우 IOS Appstore 등록 링크, Android Googleplaystore등록된 링크 필수 입력(http:// 포함 작성)
		 * template_img_attach : 친구톡 이미지 URL(영문) (jpg, png 파일만 가능, 파일 500kb 제한, 사이즈 720px * 720px, 800px *400px)
		 * template_img_link : 친구톡 이미지 링크 URL(영문) (http://를 포함하여 작성)
		 ...
		 *
		 * 친구톡과 친구톡 대체문자 발송시 야간 시간대(오후 8시 50분 ~ 다음날 8시)에는 전송할 수 없습니다.
		 * 발신프로필은 directsend.co.kr > 카카오메세지 > 발신프로필등록에서 직접 등록해야하며 비즈니스 채널 개설된 발신프로필만 등록할 수 있습니다.
		 * 템플릿 발송은 directsend.co.kr > 카카오메세지 > 템플릿 등록/관리 > 친구톡에서 직접 등록하여 사용할 수 있습니다.
		 *
		 */
         ...
         *
		 * 대체문자 발송시 주말, 연휴 기간에는 문자 인증이 다소 지연될 될 수 있으니 양해 부탁드립니다.
		...
		String kakao_plus_id = "directsend에 등록한 발신프로필 @검색용아이디";            //필수입력
		String user_template_no = "directsend에 등록한 템플릿 번호";            //템플릿 번호를 사용하지 않을 경우 주석처리 바랍니다. (하단 290 라인 API 이용하여 확인)

		//수신자 정보 추가 - 필수 입력(주소록 미사용시), 치환문자 미사용시 치환문자 데이터를 입력하지 않고 사용할수 있습니다.
		//치환문자 미사용시 "{"mobile":"01000000001"} 번호만 입력 해주시기 바랍니다.
		String receiver = "{\"name\": \"강길동\", \"mobile\":\"수신자번호\", \"note1\":\"다이렉트센드 1\",\"note2\":\"다이렉트센드 2\",\"note3\":\"다이렉트센드 3\",\"note4\":\"다이렉트센드 4\",\"note5\":\"다이렉트센드 5\"}"
		...
		// 템플릿 정보 추가
		int kakao_friend_type = 1;      // 1 : 친구톡(텍스트) / 2 : 친구톡(이미지)
		int template_ad = 1;       //필수입력 0 : (광고)미표시 / 1 : (광고)표시
		String template_title = "친구톡 제목 입니다.";
		String template_body = "#{이름}님 친구톡 알림 입니다. 전화번호 : #{휴대폰} 메모1 : #{비고1} 메모2 : #{비고2} 메모3 : #{비고3} 메모4 : #{비고4} 메모5 : #{비고5} ";

		//버튼 정보 추가 - 웹링크(WL)인 경우 모바일 링크, PC 링크 필수 입력(http:// 포함 작성), 앱링크(AL)인 경우 IOS Appstore 등록 링크, Android Googleplaystore등록된 링크 필수 입력(http:// 포함 작성)
		String template_buttons = "{\"linkType\":\"WL\",\"name\":\"웹링크\",\"linkM\":\"https://www.directsend.co.kr\",\"linkP\":\"https://www.directsend.co.kr\",\"linkI\":\"\",\"linkA\":\"\"}"
		    + ",{\"linkType\":\"AL\",\"name\":\"앱링크\",\"linkM\":\"\",\"linkP\":\"\",\"linkI\":\"directsend\",\"linkA\":\"directsend\"}"
		    + ",{\"linkType\":\"BK\",\"name\":\"봇키워드\",\"linkM\":\"\",\"linkP\":\"\",\"linkI\":\"\",\"linkA\":\"\"}"
		    + ",{\"linkType\":\"MD\",\"name\":\"메시지전달\",\"linkM\":\"\",\"linkP\":\"\",\"linkI\":\"\",\"linkA\":\"\"}"
		;

		template_buttons = "["+ template_buttons +"]";      //json array

		// 친구톡(이미지) 발송시 아래 주석을 해제하고 이미지 파일의 URL을 입력하여 주시기 바랍니다.
		// jpg, png 파일만 가능, 파일 500kb 제한, 사이즈 720px * 720px, 800px *400px 제한 1개만 가능합니다.
		String template_img_attach = "https://directsend.co.kr/jpgimg1.jpg";     //친구톡(이미지) 발송시 필수 입력
		String template_img_link = "https://www.directsend.co.kr";
		...
		// 예약발송 정보 추가
		String reserve_type = "ONETIME"; // NORMAL - 즉시발송 / ONETIME - 1회예약 / WEEKLY - 매주정기예약 / MONTHLY - 매월정기예약
		...
		//postvars = postvars+", \"kakao_friend_type\":\""+kakao_friend_type+"\"";       //템플릿 번호를 사용하지 않을 경우 필수입력
		//postvars = postvars+", \"template_ad\":\""+template_ad+"\"";                   //템플릿 번호를 사용하지 않을 경우 필수입력
		//postvars = postvars+", \"template_title\":\""+template_title+"\"";
		//postvars = postvars+", \"template_body\":\""+template_body+"\"";               //템플릿 번호를 사용하지 않을 경우 필수입력
		//postvars = postvars+", \"template_buttons\":"+template_buttons;        //템플릿 번호를 사용하지 않고 버튼을 추가할 경우 주석 해제하여 사용
		//postvars = postvars+", \"template_img_attach\":\""+template_img_attach+"\""; //친구톡 이미지 발송시 주석해제 바랍니다
		//postvars = postvars+", \"template_img_link\":\""+template_img_link+"\"";     //친구톡 이미지 발송시 주석해제 바랍니다
		...
		*/
		...
		/* status code
			...
			320 : 친구톡 발송가능한 시간이 아닙니다.
			321 : 이미지 사이즈 오류
			999 : Internal Error.
		 */

		/* 발신프로필 / 예약 API 사용시 여기까지 삭제후 이용 하시기 바랍니다. */
		...
		//String postvars = "{\"username\":\""+ username +"\",\"key\":\""+ key +"\", \"profile_type\":\"1\"}";		//JSON 데이터

		//String url = "https://directsend.co.kr/index.php/api_kakao/profile/get/list";        //URL
		...
		*/
		...
		//String postvars = "{\"username\":\""+ username +"\",\"key\":\""+ key +"\", \"template_type\":\"4\"}";		//JSON 데이터

		//String url = "https://directsend.co.kr/index.php/api_kakao/template/get/list";        //URL
		...
		*/
		...
		//String postvars = "{\"username\":\""+ username +"\",\"key\":\""+ key +"\",\"kakao_reserve\":\"1\"}";		//JSON 데이터
		...
		*/
		...
		//String postvars = "{\"username\":\""+ username +"\",\"key\":\""+ key +"\",\"kakao_reserve\":\"1\",\"reserve_no\":\""+ reserve_no +"\"}";		//JSON 데이터
